...
contract tokenTracer is usingProvable {
    ...
    function token_queryAccount(address _from, address _to, uint checkPoint) public view returns(uint _checkPoint, bytes32[] memory _transactionHash, address[] memory _sender, address[] memory _receiver, uint[] memory _value, uint[] memory _blockNumber, uint[] memory _timestamp) {
        ...
        for (_checkPoint = checkPoint; _checkPoint < transactionCount && _checkPoint < checkPoint + 100; _checkPoint++) {
            // Query for Sender
            if (msg.sender == _to && sender[_checkPoint] == _from) {
                matchIndex[size] = _checkPoint;
                size++;
            }
            // Query for Receiver
            else if (msg.sender == _from && receiver[_checkPoint] == _to) {
                matchIndex[size] = _checkPoint;
                size++;
            }
            // Query for Both
    ...
    function token_queryTime(uint startTime, uint endTime, address _from, address _to, uint checkPoint) public view returns(uint _checkPoint, bytes32[] memory _transactionHash, address[] memory _sender, address[] memory _receiver, uint[] memory _value, uint[] memory _blockNumber, uint[] memory _timestamp) {
        uint size;
        uint[] memory matchIndex = new uint[](transactionHash.length);
        if (checkPoint == 0) {
        ...
        for (_checkPoint = checkPoint; _checkPoint < transactionCount && _checkPoint < checkPoint + 100; _checkPoint++) {
            if (endTime >= timeStamp[_checkPoint]) {
                ...
                // Query for Both
                else if (_from == sender[_checkPoint] && _to == receiver[_checkPoint]) {
                    matchIndex[size] = _checkPoint;
                    size++;
                } 
                // Only query for time
                else if (msg.sender == _from && msg.sender == _to) {
                    matchIndex[size] = _checkPoint;
                    size++;
                }
            } else {
    ...
    function token_queryBlock(uint startBlock, uint endBlock, address _from, address _to, uint checkPoint) public view returns(uint _checkPoint, bytes32[] memory _transactionHash, address[] memory _sender, address[] memory _receiver, uint[] memory _value, uint[] memory _blockNumber, uint[] memory _timestamp) {
        uint size;
        uint[] memory matchIndex = new uint[](transactionHash.length);
        if (checkPoint == 0) {
